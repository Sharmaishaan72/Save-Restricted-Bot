...
async def set_string_session(client: Client, message):
	...
	if len(message.command) < 2:
		await client.send_message(message.chat.id, "__Usage__: `/string NEW_STRING_SESSION`", reply_parameters=ReplyParameters(message_id=message.id))
		return
	DATA["STRING"] = message.command[1].strip()
	with open("config.json", "w") as f:
		json.dump(DATA, f, indent=4)
	msg = await client.send_message(message.chat.id, "**STRING session updated.**", reply_parameters=ReplyParameters(message_id=message.id))
...
async def add_access(client: Client, message):
	DATA = open_config()
	if len(message.command) < 2:
		await client.send_message(message.chat.id, "__Usage__: `/addaccess {user_id}`", reply_parameters=ReplyParameters(message_id=message.id))
		return
	old = DATA['OWNER_USERIDS']
	DATA["OWNER_USERIDS"] = old + "," + message.command[1].strip()
	with open("config.json", "w") as f:
		json.dump(DATA, f, indent=4)
	msg = await client.send_message(message.chat.id, "**updating...**", reply_parameters=ReplyParameters(message_id=message.id))
	config.config.reload_env()
